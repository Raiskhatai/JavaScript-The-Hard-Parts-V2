// Asynchronous JavaScript ka kaam browser ke features ki wajah se possible hota hai.
// JavaScript khud single-threaded hoti hai, lekin browser usko async power deta hai

// 1 -> Web APIs
// Browser ke andar built-in features hote hain jo time-consuming kaam JS se alag handle karte hain.
// Examples:
// setTimeout, setInterval, fetch() / AJAX (API calls),
// DOM Events (click, scroll) ,localStorage, geolocation

// 2-> call stack
// Yahan sirf synchronous code execute hota hai.
// Ek time par ek hi function.

// Callback Queue (Task Queue)
// Web API ka kaam complete hone ke baad Uska callback yahan wait karta hai.

// JavaScript doesn't has featrues like setTimeOut() etc. let's say we apply setTimeOut then what would happen actually we request the browser for setTimeOut and timer it's label for browser it will request to the internet and then he will return us. setTimeOut();
